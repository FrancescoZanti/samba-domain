This repository is a fork from https://github.com/Fmstrat/samba-domain.
The idea is to support the creation of X86_64 and ARM images for Docker, so for example You are able to run a Samba domain on a Raspberry Pi.

Now I am supporting Ubuntu and Fedora is WIP.
...
git clone https://github.com/FrancescoZanti/samba-domain.git
cd samba-domain
docker build -t samba-domain -f Dockerfile-Ubuntu .
```

Or just use the HUB:

```
docker pull francescozanti/samba-domain
```
